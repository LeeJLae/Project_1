...
def single_proc_run(local_rank, main_port, cfg, world_size):
    ...
    if checkpoint_path and os.path.exists(checkpoint_path):
        try:
            ...
            trainer.model.load_state_dict(checkpoint, strict=False)  # strict=False로 불일치 파라미터 무시
            print(f"Loaded checkpoint from {checkpoint_path}")
        except RuntimeError as e:
            print(f"Error loading checkpoint: {e}")
    else:
        print("No checkpoint found or path is not provided.")

    trainer.run()

    # 모델 파라미터 저장 경로 설정
    output_dir = cfg.launcher.experiment_log_dir
    ...
    if hasattr(trainer, 'model'):
        ...
        print("Error: 'trainer' object has no attribute 'model'. Cannot save model parameters.")
...
def main(args) -> None:
    # 절대 경로로 config 디렉터리 설정
    with initialize_config_dir(config_dir="/home/jovyan/Project/STEP2/SAM/SAM2_CODE/sam2/configs", version_base="1.2"):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        required=True,
        type=str,
        help="path to config file (e.g. sam2.1_training/sam2.1_finetune.yaml)",
    )
    parser.add_argument(
        "--use-cluster",
        type=int,
    ...
    # Register custom resolvers for OmegaConf if any
    register_omegaconf_resolvers()
    
    # Start main function with parsed arguments
    main(args)
